...
| Oracle | — | :white_check_mark: | 연습문제에서 Oracle 문법 탭 제공 |
| SQL Server | — | :white_check_mark: | 연습문제에서 T-SQL 문법 탭 제공 |

!!! tip "Oracle / SQL Server 사용자"
    Oracle과 SQL Server는 현재 연습문제의 SQL 탭으로 지원됩니다. DB별 문법이 다른 문제에서 Oracle(`NVL`, `FETCH FIRST N ROWS ONLY`, 시퀀스 등)과 SQL Server(`TOP`, `ISNULL`, `CONVERT` 등)의 정답을 확인할 수 있습니다. 데이터 생성기의 Oracle/SQL Server 익스포터는 향후 추가 예정입니다.

### 향후 지원 계획

| 데이터베이스 | 상태 | 비고 |
|------------|:----:|------|
| Oracle 데이터 생성 | 계획 중 | 익스포터 추가 예정 (PL/SQL, 시퀀스 등) |
| SQL Server 데이터 생성 | 계획 중 | 익스포터 추가 예정 (T-SQL, IDENTITY 등) |
...
| 버전 | 날짜 | 주요 변경 |
|:----:|:----:|----------|
| v3.3 | 2026-04-15 | Oracle / SQL Server 연습문제 SQL 탭 추가, 5개 DB 지원 |
