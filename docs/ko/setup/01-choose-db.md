...
이 튜토리얼은 **SQLite, MySQL, PostgreSQL** 세 가지 데이터베이스를 지원합니다. 모든 레슨과 연습 문제에 DB별 탭이 제공되므로, 어떤 DB를 선택해도 동일한 내용을 학습할 수 있습니다.
...
|-|--------|----------------|------------|
| **난이도** | 매우 쉬움 | 보통 | 보통 |
| **설치** | 불필요 (파일 기반) | 서버 설치 필요 | 서버 설치 필요 |
| **적합한 용도** | 학습, 임베디드, 모바일 | 웹 서비스, CRUD 앱 | 분석, 복잡한 쿼리, GIS |
| **SQL 표준** | 대부분 지원 | 일부 미지원 | 최고 수준 |
| **저장 프로시저** | 미지원 | 지원 | 지원 |
| **JSON** | 기본 함수 | JSON 타입 | JSONB (고성능) |
