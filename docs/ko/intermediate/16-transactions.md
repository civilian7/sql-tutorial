...
앞선 15강에서 DDL로 테이블을 만들고 변경하는 방법을 배웠습니다. 실제 운영 환경에서는 여러 SQL 문이 **하나의 논리적 작업**을 이루는 경우가 대부분입니다. 예를 들어 주문을 처리할 때는 주문 INSERT, 주문 상세 INSERT, 결제 INSERT, 재고 UPDATE가 모두 성공해야 의미가 있습니다. 하나라도 실패하면? 전부 없던 일로 되돌려야 합니다. 이것이 **트랜잭션**입니다.
...
### 연습 5
SAVEPOINT를 사용하여 다음 시나리오를 작성하세요: 주문(id=6001)에 상품 3개를 추가하되, 두 번째 상품 추가 후 문제가 발견되어 두 번째 상품만 취소하고, 나머지는 유지한 채 커밋합니다.
