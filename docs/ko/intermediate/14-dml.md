...
[13강](13-union.md)에서 UNION으로 여러 쿼리 결과를 합치는 법을 배웠습니다. 지금까지는 SELECT로 데이터를 읽기만 했습니다. 이번에는 데이터를 직접 추가(INSERT), 수정(UPDATE), 삭제(DELETE)하는 방법을 배웁니다. [0강](../beginner/00-introduction.md)에서 배운 CRUD 중 C, U, D에 해당합니다.

!!! note "이미 알고 계신다면"
    INSERT, UPDATE, DELETE에 익숙하다면 [15강: DDL](15-ddl.md)로 건너뛰세요.
...
| 개념 | 설명 | 예시 |
|------|------|------|
| INSERT INTO ... VALUES | 행 추가 (단일/다중) | `INSERT INTO t (a, b) VALUES (1, 2)` |
| INSERT INTO ... SELECT | 다른 테이블에서 복사하여 삽입 | `INSERT INTO t SELECT ... FROM s` |
| UPDATE SET ... WHERE | 조건에 맞는 행 수정 | `UPDATE t SET a = 1 WHERE id = 5` |
| DELETE FROM ... WHERE | 조건에 맞는 행 삭제 | `DELETE FROM t WHERE id = 5` |
| 트랜잭션 | BEGIN / COMMIT / ROLLBACK | 여러 DML을 원자적으로 실행 |
| UPSERT | 존재하면 UPDATE, 없으면 INSERT | `ON CONFLICT DO UPDATE` (SQLite/PG) |
| 안전 수칙 | UPDATE/DELETE 전 SELECT로 확인 | WHERE 누락 시 전체 행 영향 |
...
!!! tip "채점 기준"
    ...
    |------|------|
    | 연습 1: 다중 행 INSERT | 8점 |
    | 연습 2: WHERE 누락 위험 설명 + 올바른 UPDATE | 8점 |
    | 연습 3: UPSERT (ON CONFLICT) | 10점 |
    | 연습 4: UPSERT + 조건부 가격 유지 | 12점 |
    | 연습 5: 단일 행 INSERT | 6점 |
    | 연습 6: SELECT 확인 후 UPDATE | 8점 |
    | 연습 7: 복합 WHERE 조건 UPDATE | 8점 |
    | 연습 8: 복합 WHERE 조건 UPDATE | 8점 |
    | 연습 9: SELECT 확인 후 DELETE | 8점 |
    | 연습 10: INSERT ... SELECT 활용 | 10점 |
    | 연습 11: SELECT 확인 후 DELETE | 6점 |
    | 연습 12: 서브쿼리 활용 UPDATE | 8점 |
    | **합계** | **100점** |
