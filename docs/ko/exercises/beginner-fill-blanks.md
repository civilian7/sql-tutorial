...
**힌트:** 빈칸에는 grade 컬럼과 'VIP' 값을 비교하는 등호 조건이 들어갑니다.
...
**힌트:** 빈칸에는 정렬 기준 컬럼과 내림차순 키워드(`DESC`)가 들어갑니다.
...
**힌트:** 빈칸에는 그룹화된 결과를 필터링하는 `HAVING` 절이 들어갑니다. `WHERE`는 그룹화 전, `HAVING`은 그룹화 후 필터링.
...
**힌트:** 빈칸에는 두 테이블의 연결 조건이 들어갑니다. products의 외래키와 categories의 기본키를 매칭.
...
**힌트:** `LEFT JOIN` 후 매칭되지 않은 행은 오른쪽 테이블 컬럼이 NULL. `IS NULL`로 확인.
...
**힌트:** 빈칸에는 `BETWEEN '시작일' AND '종료일'`로 날짜 범위를 지정하세요.
...
**힌트:** `WHEN stock_qty = 0 THEN '품절'` 식으로 구간별 조건을 나열하세요. 마지막은 `ELSE`.
...
**힌트:** `COALESCE(컬럼, 기본값)` 함수는 NULL일 때 대체값을 반환합니다.
...
**힌트:** 빈칸에는 평균 가격을 구하는 서브쿼리 `(SELECT AVG(price) FROM products)`가 들어갑니다.
...
**힌트:** 세 빈칸에는 각각 `COUNT(*)`, `ROUND(AVG(...), 0)`, `MAX(...)` 집계 함수가 들어갑니다.
...
**힌트:** `LIKE` 패턴에서 `%`는 임의 문자열. `@testmail.kr`로 끝나는 패턴을 만드세요.
...
**힌트:** `SUBSTR(ordered_at, 시작위치, 길이)`로 문자열을 잘라냅니다. 연도는 1~4자리, 월은 6~2자리.
...
**힌트:** 여러 값 중 하나와 일치하는지 확인할 때 `IN ('값1', '값2')` 연산자를 사용하세요.
...
**힌트:** 각 빈칸에는 외래키 관계가 들어갑니다. `o.customer_id = c.id`, `o.id = p.order_id`, `o.id = sh.order_id` 패턴.
