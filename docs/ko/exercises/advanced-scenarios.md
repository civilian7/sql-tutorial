...
역할이 주어집니다. 해당 부서가 실제로 필요로 하는 보고서를 직접 작성하세요.

---

### 시나리오 1 — CEO 주간 보고

**역할:** 당신은 데이터 분석가입니다. CEO가 월요일 아침 회의에서 볼 주간 요약을 요청했습니다.

**요구사항:** 지난 주(2024-12-16 ~ 2024-12-22) 기준으로 주문 수, 매출, 평균 주문 금액, 신규 가입 고객 수, 전주 대비 매출 증감률을 한 번에 보여주세요.

**힌트:** 이번 주/지난 주/신규 고객을 각각 CTE로 계산한 뒤 `FROM this_week, last_week, new_customers`로 한 행에 합칩니다.
...
**역할:** 당신은 MD(상품기획)팀 담당자입니다.

**요구사항:** 2024년 매출 상위 20개 상품에 대해 상품명, 카테고리, 매출, 판매 수량, 평균 평점, 반품 건수를 한 눈에 보여주세요. 평점이 낮거나 반품이 많은 상품은 주의가 필요합니다.

**힌트:** 매출 Top 20을 CTE로 먼저 뽑고, `reviews`와 `returns`를 `LEFT JOIN`으로 붙이세요. `CASE`로 경고 플래그를 추가합니다.
...
??? success "정답"
    ...
    SELECT
        ...
        CASE
            WHEN pr.avg_rating < 3.5 THEN '⚠ 평점 주의'
            WHEN ret.return_count > 5 THEN '⚠ 반품 주의'
            ELSE ''
        END AS alert
...
**역할:** 당신은 마케팅팀 담당자입니다. 재활성화 이메일 캠페인을 준비합니다.

**요구사항:** 과거 3회 이상 구매했지만 최근 6개월간 주문이 없는 고객 리스트를 뽑아주세요. 이름, 이메일, 등급, 마지막 주문일, 총 구매 금액을 포함하고, 총 구매 금액이 높은 순으로 정렬하세요.

**힌트:** `HAVING COUNT(*) >= 3 AND MAX(ordered_at) < DATE('기준일', '-6 months')`로 두 조건을 동시에 필터링합니다.
...
**역할:** 당신은 재무팀 담당자입니다. 2024년 12월 월말 마감 보고서를 준비합니다.

**요구사항:** 결제 수단별 건수, 총 결제 금액, 환불 건수, 환불 금액, 순매출(결제-환불)을 보여주세요.

**힌트:** `payments` 테이블에서 `SUM(CASE WHEN status = 'completed' ...)`와 `SUM(CASE WHEN status = 'refunded' ...)`로 조건부 집계합니다.
...
**요구사항:** 출고 후 3일 이상 걸린 배송 건을 택배사별로 집계하세요. 택배사명, 지연 건수, 평균 배송일, 최대 배송일을 보여주세요.

**힌트:** `JULIANDAY(delivered_at) - JULIANDAY(shipped_at)`로 배송 소요일을 계산하고, `>= 3` 조건으로 지연 건만 필터링합니다.
...
**요구사항:** 최근 30일간 일평균 판매량 대비 현재 재고가 14일 미만인 상품 리스트를 만드세요. 공급업체 연락처도 포함합니다.

**힌트:** CTE로 30일간 일평균 판매량을 계산한 뒤, `stock_qty / avg_daily_sales < 14`로 필터링하고 `suppliers` JOIN으로 연락처를 붙이세요.
...
**요구사항:** 7일 이상 미해결 문의 중 VIP/GOLD 고객의 건을 우선순위와 함께 보여주세요. 해당 고객의 총 구매 금액도 포함하여 중요도를 판단합니다.

**힌트:** `complaints` JOIN `customers`에서 `status = 'open'`과 `grade IN ('VIP', 'GOLD')`로 필터링. 서브쿼리로 고객별 총 구매액을 LEFT JOIN합니다.
...
### 시나리오 8 — 경영진 연간 KPI 대시보드

**역할:** 당신은 데이터 분석가입니다. 연말 경영 회의용 KPI를 준비합니다.

**요구사항:** 2024년 핵심 지표를 한 행으로 요약하세요 — 총 매출, 주문 수, 신규 고객 수, 활성 고객 수(1회 이상 구매), 평균 객단가, 취소율, 반품률.

**힌트:** 각 KPI를 스칼라 서브쿼리 `(SELECT ... FROM ...)` 로 SELECT 절에 나열하면 한 행으로 출력됩니다.
