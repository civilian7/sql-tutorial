...
지금까지 배운 내용을 총동원하여 쇼핑몰의 매출 성과에 관한 다섯 가지 현실적인 비즈니스 질문에 답해 보세요. 각 질문은 이전 질문을 기반으로 합니다. 정답을 확인하기 전에 먼저 쿼리를 직접 작성해 보세요.

이 연습 문제는 의도적으로 열린 형식으로 설계되었습니다. 유효한 접근 방법이 하나 이상일 수 있습니다. 먼저 올바른 결과를 얻는 데 집중하고, 그 다음에 명확성과 효율성을 고민하세요.

---

## 질문 1 — 월별 매출 추이 (2022–2024)

**비즈니스 질문:** CEO가 지난 3년간의 월별 매출 보고서를 요청했습니다. 각 월의 매출, 주문 수, 평균 주문 금액을 보여주세요. 취소 및 반품 주문은 제외합니다.

**힌트:**
- 월 추출에는 `SUBSTR(ordered_at, 1, 7)` 사용
- `SUM`, `COUNT`, `AVG` 활용
...
??? success "정답"
    ...
    **예상 결과 형태:** 36개 행 (월 1개씩), 매출은 시간이 지남에 따라 증가하며 매년 11~12월에 최고점을 기록합니다.

---

## 질문 2 — 2024년 매출 상위 10개 상품

**비즈니스 질문:** MD팀이 2024년에 가장 많은 매출을 올린 상품이 무엇인지 알고 싶어합니다. 상품명, 카테고리, 판매 수량, 총 매출, 평균 고객 평점이 필요합니다.

**힌트:**
- `order_items` → `orders` → `products` → `categories` 순으로 JOIN
...
**비즈니스 질문:** 이 쇼핑몰에 계절성 매출 패턴이 있나요? 전체 연도에 걸쳐 달력 월(1월~12월)별 평균 월 매출을 계산하세요. 가장 높은 달과 가장 낮은 달을 찾아보세요.

**힌트:**
- 월 번호(01~12) 추출에는 `SUBSTR(ordered_at, 6, 2)` 사용
...
??? success "정답"
    ...
    **예상 인사이트:** 11월과 12월이 가장 높은 평균을 기록할 것입니다 (연말 쇼핑 시즌). 7~8월에는 소폭 하락이 있을 수 있습니다. 3월에는 봄 신학기 특수로 두 번째 피크가 나타납니다.

---

## 질문 4 — 결제 수단 분석

**비즈니스 질문:** 재무팀이 고객의 결제 수단을 파악하고 싶어합니다. 결제 수단별로 거래 수, 총 결제금액, 평균 거래금액, 그리고 전체 매출에서 차지하는 비율을 보여주세요.

**힌트:**
- `payments`를 `orders`와 JOIN하여 주문 날짜 포함
...
??? success "정답"
    ...
    **예상 인사이트:** 카드 결제가 압도적인 비중을 차지할 것입니다. 간편결제(kakao_pay, naver_pay)는 평균 거래금액이 높을 것입니다. 포인트 전액 결제는 평균 금액이 매우 낮을 것입니다.

---

## 질문 5 — 카테고리별 전년 대비 매출 성장률(YoY)

**비즈니스 질문:** 이사회가 2023년과 2024년의 카테고리별 YoY 성장률 보고서를 원합니다. 각 카테고리의 두 연도 매출과 변화율을 보여주세요.

**힌트:**
- 연도별 매출에는 조건부 집계 `SUM(CASE WHEN ... THEN ... END)` 사용
- 또는 연도별 CTE 두 개를 `category_id`로 JOIN하는 방법 사용
...
??? success "정답"
    ...
    **예상 인사이트:** AI/생산성 관련 하드웨어(노트북, 스토리지)와 연관된 카테고리가 강한 성장세를 보일 것입니다. 범용 액세서리는 시장 포화로 인해 성장이 정체되거나 역성장할 수 있습니다.

---

## 보너스 도전 과제

질문 3과 5를 결합하세요. 2024년 **카테고리(행)** × **월(열)** 형태의 히트맵 표를 만들어 보세요. 12개의 `SUM(CASE WHEN month = 'XX' ...)` 컬럼을 가진 조건부 집계를 사용합니다.
