...
**힌트:** `COUNT(*)`와 `SUM(total_amount)`를 함께 사용하세요.
...
**힌트:** `GROUP BY status`와 `COUNT(*)`를 사용하세요.
...
**힌트:** `WHERE ordered_at LIKE '2024%'`로 2024년 데이터만 필터링하세요.
...
**힌트:** 취소율은 `100.0 * COUNT(*) / (SELECT COUNT(*) FROM orders)`로 계산. 서브쿼리로 전체 수를 구하세요.
...
**힌트:** `ORDER BY total_amount DESC`로 정렬 후 `LIMIT 10`.
...
**힌트:** `SUBSTR(ordered_at, 1, 7)`로 'YYYY-MM'을 추출하고 `GROUP BY`로 월별 집계. `WHERE`로 2024년 필터링.
...
**힌트:** `AVG(total_amount)`와 `AVG(CASE WHEN status NOT IN ('cancelled') THEN total_amount END)`를 한 쿼리에서 비교.
...
**힌트:** `SUM(CASE WHEN shipping_fee = 0 THEN 1 ELSE 0 END)`로 조건부 카운트. 전체 `COUNT(*)`로 나눠 비율 계산.
...
**힌트:** `WHERE point_used > 0`으로 적립금 사용 주문만 필터링. `AVG(point_used)`로 평균 계산.
...
**힌트:** `payments` 테이블에서 `GROUP BY method`로 그룹화. `COUNT(*)`와 `SUM(amount)` 사용.
...
**힌트:** `WHERE method = 'card'`로 카드 결제만 필터. `CASE WHEN`으로 할부 건수를 세고, 조건부 `AVG`로 평균 개월 계산.
...
**힌트:** `STRFTIME('%w', ordered_at)`로 요일 번호(0=일, 1=월, ...)를 추출. `CASE`로 한글 요일명 매핑.
...
**힌트:** `CASE WHEN notes IS NOT NULL THEN 1 ELSE 0 END`로 메모 유무를 판별하고 `SUM`으로 집계.
...
**힌트:** `WHERE status NOT IN ('cancelled')`로 취소 제외. `SUBSTR(ordered_at, 1, 4)`로 연도 추출 후 `GROUP BY`.
...
**힌트:** `payments` 테이블에서 `WHERE status = 'refunded'`로 필터링. `COUNT(*)`와 `SUM(amount)` 사용.
