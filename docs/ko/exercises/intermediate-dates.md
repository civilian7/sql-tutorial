...
**힌트:** `SUBSTR(ordered_at, 1, 7)`로 월을 추출하고, `WHERE ordered_at LIKE '2025%'`로 연도 필터링.
...
**힌트:** `CASE WHEN SUBSTR(ordered_at, 6, 2) IN ('01','02','03') THEN 'Q1' ...`으로 월을 분기로 변환.
...
**힌트:** 서브쿼리에서 `MIN(ordered_at)`으로 첫 주문일을 구한 뒤, `JULIANDAY` 차이로 일수 계산.
...
**힌트:** `SUBSTR(ordered_at, 12, 2)`로 시간 부분을 추출. `CAST(... AS INTEGER)`로 정수 변환.
...
**힌트:** `BETWEEN '2025-11-01' AND '2025-11-30 23:59:59'`로 기간 필터링. 시간 끝까지 포함하도록 주의.
...
**힌트:** 서브쿼리에서 `JULIANDAY` 차이로 소요일을 계산한 뒤, `CASE WHEN days <= 1 THEN ...`으로 구간 분류.
...
**힌트:** `MAX(ordered_at)`로 마지막 주문일을 구하고, `JULIANDAY('기준일') - JULIANDAY(MAX(...))`로 경과일 계산. `HAVING`으로 180일 필터.
...
**힌트:** `STRFTIME('%w', ordered_at)`로 요일 번호를 추출하고, `CASE`로 한글 요일명 변환. 두 컬럼으로 `GROUP BY`.
...
**힌트:** CTE로 연도별 매출을 구한 뒤, `LAG(revenue) OVER (ORDER BY year)`로 전년 매출을 참조.
...
**힌트:** `returns` 테이블에서 `JULIANDAY(completed_at) - JULIANDAY(requested_at)`로 소요일 계산. `WHERE status = 'completed'`.
...
**힌트:** CTE로 고객별 `MIN(ordered_at)`(첫 주문일)을 구한 뒤, 주문월과 첫 주문월을 비교하여 신규/재구매 구분.
...
**힌트:** `products JOIN product_prices`로 이력 연결. `HAVING COUNT >= 2`. 최초 가격은 서브쿼리에서 `ORDER BY started_at ASC LIMIT 1`.
