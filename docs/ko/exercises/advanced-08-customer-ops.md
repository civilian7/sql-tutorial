...
??? tip "힌트"
    - 이탈 기준: `JULIANDAY('2025-03-31') - JULIANDAY(MAX(ordered_at)) > 180`
    - 등급별 GROUP BY
...
??? success "정답"
    ...
    |---|---|---|---|---|---|
    | VIP | 활성 | 85 | 1200000 | 15.2 | 25 |
    | VIP | 이탈 위험 | 12 | 980000 | 8.5 | 245 |
    | GOLD | 활성 | 320 | 850000 | 10.1 | 35 |
    | GOLD | 이탈 위험 | 45 | 720000 | 5.2 | 220 |
    | ... | ... | ... | ... | ... | ... |
...
??? tip "힌트"
    - 활동 기간 = (마지막 주문일 - 첫 주문일) / 365. 최소 1년으로 처리
...
??? tip "힌트"
    - `order_items` + `orders`에서 최근 6개월 주문 확인
...
??? success "정답"
    ...
    |---|---|---|---|---|---|
    | (구형 부품) | 저장장치 | 45 | 13500000 | 2024-05-10 | 325 |
    | (단종 예정) | 메모리 | 30 | 9000000 | 2024-06-22 | 282 |
    | ... | ... | ... | ... | ... | ... |
...
??? tip "힌트"
    - 최근 3개월 출고량: `inventory_transactions`에서 `type = 'outbound'`
...
??? success "정답"
    ...
    |---|---|---|---|---|---|
    | (인기 SSD) | 8 | 2.5 | 26 | 3 | 즉시 발주 |
    | (인기 메모리) | 12 | 1.8 | 19 | 7 | 즉시 발주 |
    | ... | ... | ... | ... | ... | ... |
...
??? tip "힌트"
    - 매출원가(COGS): `SUM(oi.quantity * p.cost_price)` (2024년 판매분)
...
??? success "정답"
    ...
    |---|---|---|---|---|---|
    | (인기 카테고리) | 50 | 2500000000 | 350000000 | 7.1 | 51 |
    | (일반 카테고리) | 80 | 1800000000 | 500000000 | 3.6 | 101 |
    | ... | ... | ... | ... | ... | ... |
...
??? tip "힌트"
    - `inventory_transactions`의 `type`: inbound(양수), outbound(음수)
    - 조건부 집계로 입고/출고 분리
...
??? tip "힌트"
    - `returns` 테이블: `reason`, `refund_amount`, `requested_at`
...
??? success "정답"
    ...
    |---|---|---|---|---|---|---|
    | 박지은 | 85 | 95.3 | 0.8 | 72.5 | 65.0 | 1.5 |
    | 김하늘 | 72 | 91.7 | 1.2 | 68.0 | 65.0 | 1.5 |
    | ... | ... | ... | ... | ... | ... | ... |
...
??? success "정답"
    ...
    |---|---|---|---|---|---|
    | (문제 상품 A) | 120 | 15 | 12 | 27 | 22.5 |
    | (문제 상품 B) | 85 | 8 | 10 | 18 | 21.2 |
    | (정상 상품 C) | 200 | 5 | 3 | 8 | 4.0 |
    | ... | ... | ... | ... | ... | ... |
...
??? tip "힌트"
    - `suppliers` → `products` → 각종 테이블 JOIN
...
??? success "정답"
    ...
    |---|---|---|---|---|---|---|
    | (주)테크솔루션 | 45 | 2500000000 | 3200 | 5.5 | 4.2 | 1500 |
    | 디지털파트너 | 38 | 2100000000 | 2800 | 4.2 | 4.0 | 1200 |
    | ... | ... | ... | ... | ... | ... | ... |
...
??? tip "힌트"
    - 각 영역을 별도 CTE로 작성한 뒤 CROSS JOIN
