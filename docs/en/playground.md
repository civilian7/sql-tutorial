...
Run SQL queries directly in your browser. No server connection required.

!!! info "Dataset"
    This playground uses a reduced version of the tutorial database (~200 rows per table).
    For the full dataset (750K+ rows), see [Generate Database](setup/03-generate.md).
    ...
    <div class="pg-container">
      ...
      <div class="pg-main">
        <textarea class="pg-editor" id="pg-editor" spellcheck="false"
          placeholder="Enter your SQL query...">SELECT * FROM customers LIMIT 10;</textarea>
        <div class="pg-toolbar">
          <button class="pg-btn pg-btn-run" id="pg-run">Run</button>
          <button class="pg-btn pg-btn-reset" id="pg-reset">Reset</button>
          <span class="pg-shortcut-hint">Ctrl+Enter to run</span>
        </div>
        <div class="pg-samples">
          <span style="font-size:0.78rem; color:var(--md-default-fg-color--light,#999); margin-right:4px;">Examples:</span>
          <button class="pg-sample-btn" data-sql="SELECT * FROM customers LIMIT 10;">Browse customers</button>
          <button class="pg-sample-btn" data-sql="SELECT name, price FROM products ORDER BY price DESC LIMIT 5;">Top 5 expensive</button>
          <button class="pg-sample-btn" data-sql="SELECT grade, COUNT(*) AS cnt FROM customers GROUP BY grade;">Customers by grade</button>
...
(function() {
  ...
  async function initDB() {
    ...
    try {
      ...
      loading.innerHTML = '<div class="pg-error">Failed to load database: ' + escapeHtml(e.message) + '</div>';
    }
  }
  ...
  function showTableInfo(tableName, li) {
    ...
    let html = '<table><tr><th>Column</th><th>Type</th><th>PK</th></tr>';
    res[0].values.forEach(r => {
      html += '<tr><td>' + escapeHtml(r[1]) + '</td><td>' + escapeHtml(r[2] || '-') + '</td><td>' + (r[5] ? 'Y' : '') + '</td></tr>';
    });
    html += '</table>';
    info.innerHTML = html;
    info.style.display = '';
  ...
  function executeQuery() {
    ...
    try {
      ...
      if (!results.length) {
        resultDiv.innerHTML = '<div class="pg-status">Query executed. No rows returned. (' + elapsed + 'ms)</div>';
        return;
      }

      let html = '';
      results.forEach((res, idx) => {
        if (results.length > 1) html += '<div class="pg-status" style="margin-bottom:4px;">Result ' + (idx + 1) + '</div>';
      ...
      html += '<div class="pg-status">' + results[0].values.length + ' rows returned (' + elapsed + 'ms)</div>';
